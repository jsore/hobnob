/**
 * ../spec/cucumber/steps/response.js
 */
...
function (payloadType) {
  ...
  // const cHeaders = this.response.headers;
  // const contentType = this.response.headers['Content-Type'] || this.response.headers['content-type'];
  // const contentType = cHeaders['Content-Type'] || cHeaders['content-type'];
  ...
  if (payloadType === 'JSON object' || payloadType === 'array') {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      throw new Error(
        'Response not of Content-Type application/json'
      );
    }
    ...
    if (!contentType || !contentType.includes('text/plain')) {
      throw new Error(
        'Response not of Content-Type text/plain'
      );
    }
...
function (responseProperty, contextProperty) {
  ...
  assert.deepEqual(
    objectPath.get(
      this.responsePayload, (responseProperty === 'root' ? '' : responseProperty)
    ),
    objectPath.get(
