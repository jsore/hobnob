# hobnob/spec/cucumber/features/main.feature
#
# testing scenarios that apply everywhere, on all endpoints

Feature: General

  Scenario Outline: POST, PUT and PATCH requests should have non-empty payloads

  All POST, PUT and PATCH requests must have non-zero values
  for its "Content-Length" header

    When the client creates a <method> request to /users
    And attaches a generic empty payload
    And sends the request
    Then our API should respond with a 400 HTTP status codes
  ...
  All requests which has non-zero values for its
  "Content-Length" header must have its "Content-Type"
  header set

    When the client creates a POST request to /users
    And attaches a generic JSON payload
  ...
  All requests which has a "Content-Type" header must set
  its value to contain "application/json"
    ...
    And the payload of the response should be a JSON object
    And contains a mesage property which says 'The "Content-Type" header must always be "application/json"'
