# hobnob/spec/cucumber/features/profile/update/main.feature
...
Feature: Update User Profile
  ...
  If the client sends a PATCH request to /users/:userId/profile
  with an unsupported payload, it should receive a response
  with a 4xx HTTP status code.

    # When the client creates a PATCH request to /users/user-id/profile
    When the client creates a PATCH request to /users/:userId/profile
    And attaches a generic <payloadType> payload
    And sends the request
    Then our API should respond with a <statusCode> HTTP status code
    And the payload of the response should be a JSON object
    And contains a mesage property which says <message>
  ...
  Scenario: Non-existent User
    ...
    And the payload of the response should be JSON object
  ...
  Scenario Outline: Valid Profile
    ...
    Then our API should respond with a 200 HTTP status code
    And the payload of the response should be a JSON object
    And the root property of the response should be an object with the value {"email":"e@ma.il","password":"hunter2","profile":{"name":{"first":"Daniel","last":"Li","middle":"d4nyll"},"bio":"bio","summary":"summary"}}
